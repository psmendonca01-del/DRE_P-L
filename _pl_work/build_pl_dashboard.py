...
function fillMonthChecks(){const values=periodsForYear(state.year);$('monthChecks').innerHTML=`<label><input type="checkbox" value="all"> Todos</label>`+values.map(v=>`<label><input type="checkbox" value="${v}"> ${monthLabel(v)}</label>`).join('');applyMonthSelection();document.querySelectorAll('#monthChecks input').forEach(i=>i.onchange=()=>{let checked=[...document.querySelectorAll('#monthChecks input:checked')].map(x=>x.value);if(checked.includes('all')&&checked.length>1)checked=checked.filter(v=>v!=='all');state.months=checked.length?checked:['all'];applyMonthSelection();render()})}
function applyMonthSelection(){document.querySelectorAll('#monthChecks input').forEach(i=>i.checked=state.months.includes(i.value));$('monthSummary').textContent=state.months.includes('all')?'Todos':state.months.length===1?shortMonth(state.months[0]):'Diversos'}
...
function renderBudgetKpis(){const periods=budgetComparePeriods(),actual=sumRows(scopedFinance(periods)),budget=sumRows(scopedBudget(periods));const variance=(key)=>(actual[key]||0)-(budget[key]||0),pct=(key)=>variationPct(actual[key]||0,budget[key]||0);const label=periodRangeLabel(periods);const cards=[
...
function renderBudgetMonthly(){const data=budgetMonthlyData();const w=760,h=260,pad=34,bottom=44,top=28,plot=h-top-bottom;const max=Math.max(...data.flatMap(d=>[Math.abs(d.actual.netRevenue||0),Math.abs(d.budget.netRevenue||0),Math.abs(d.actual.ebitda||0),Math.abs(d.budget.ebitda||0)]),1);const step=(w-pad*2)/Math.max(data.length,1),bar=Math.min(18,step*.24);let bars='',line=[];data.forEach((d,i)=>{const x=pad+i*step+step/2,z=h-bottom,actual=d.actual.netRevenue||0,budget=d.budget.netRevenue||0,ebitda=d.actual.ebitda||0,bha=Math.abs(actual)/max*(plot-32),bhb=Math.abs(budget)/max*(plot-32),ye=z-ebitda/max*(plot-32);line.push(`${x},${ye}`);bars+=`<rect x="${x-bar-2}" y="${z-bha}" width="${bar}" height="${bha}" rx="5" fill="#0d5be1"></rect><rect x="${x+2}" y="${z-bhb}" width="${bar}" height="${bhb}" rx="5" fill="#0aa36f" opacity=".75"></rect><text x="${x}" y="${h-18}" text-anchor="middle" font-size="10" font-weight="750" fill="#63708d">${shortMonth(d.period)}</text>`});$('budgetMonthlyChart').innerHTML=`<svg viewBox="0 0 ${w} ${h}" class="chart"><line x1="${pad}" x2="${w-pad}" y1="${h-bottom}" y2="${h-bottom}" stroke="#dce5f3"/><line x1="${pad}" x2="${w-pad}" y1="${top}" y2="${top}" stroke="#dce5f3"/><text x="${pad}" y="${top-9}" font-size="9" font-weight="750" fill="#63708d">Valores em R$ mil</text>${bars}<polyline points="${line.join(' ')}" fill="none" stroke="#7c3aed" stroke-width="4"/></svg><div class="legend"><span><i class="sw" style="background:#0d5be1"></i>Realizado RL</span><span><i class="sw" style="background:#0aa36f"></i>Budget RL</span><span><i class="sw" style="background:#7c3aed"></i>EBITDA realizado</span></div>`}
function renderBudgetTable(){const periods=budgetComparePeriods(),actual=sumRows(scopedFinance(periods)),budget=sumRows(scopedBudget(periods)),rows=plLines(actual);const cell=(value,net)=>`${fmtMoney(value)}<span class="sub">% RL ${net?fmtPct(Math.abs(value)/net):'-'}</span>`;$('budgetTable').innerHTML=`<thead><tr><th>Linha P&L</th><th>Realizado</th><th>Budget</th><th>Variação R$</th><th>Variação %</th></tr></thead><tbody>${rows.map(line=>{const key=line[1],a=actual[key]||0,b=budget[key]||0,d=a-b,p=variationPct(a,b);return `<tr class="${line[3]?'total-row':line[4]?'group-row':''}"><td>${line[0]}</td><td class="${cls(a)}">${cell(a,actual.netRevenue)}</td><td class="${cls(b)}">${cell(b,budget.netRevenue)}</td><td class="${cls(d)}">${fmtMoney(d)}</td><td class="${Number.isFinite(p)?cls(p):''}">${Number.isFinite(p)?fmtPct(p):'-'}</td></tr>`}).join('')}</tbody>`}
function renderBudgetByClient(){const periods=budgetComparePeriods();const groups=new Map();for(const r of filteredBudgetRows(periods)){if(!groups.has(r.client))groups.set(r.client,[]);groups.get(r.client).push(r)}const actualGroups=new Map();for(const r of filteredDetailRows(periods)){if(!actualGroups.has(r.client))actualGroups.set(r.client,[]);actualGroups.get(r.client).push(r)}const names=[...new Set([...groups.keys(),...actualGroups.keys()])].filter(Boolean);const data=names.map(name=>{const a=financeFromDetails(actualGroups.get(name)||[]),b=financeFromDetails(groups.get(name)||[]);return {name,a:a.netRevenue||0,b:b.netRevenue||0,d:(a.netRevenue||0)-(b.netRevenue||0)}}).filter(x=>Math.abs(x.a)>0||Math.abs(x.b)>0).sort((x,y)=>Math.abs(y.d)-Math.abs(x.d)).slice(0,12);$('budgetClientTable').innerHTML=`<thead><tr><th>Cliente</th><th>Realizado RL</th><th>Budget RL</th><th>Variação</th></tr></thead><tbody>${data.map(r=>`<tr><td>${displayLabel(r.name)}</td><td class="${cls(r.a)}">${fmtMoney(r.a)}</td><td class="${cls(r.b)}">${fmtMoney(r.b)}</td><td class="${cls(r.d)}">${fmtMoney(r.d)}</td></tr>`).join('')}</tbody>`}
function renderBudgetView(){renderBudgetKpis();renderBudgetMonthly();renderBudgetTable();renderBudgetByClient()}
