...
function years(){return [...new Set(DATA.meta.periods.map(p=>p.slice(0,4)))].sort((a,b)=>b.localeCompare(a))}
function periodsForYear(y){return DATA.meta.periods.filter(p=>p.startsWith(y+'-')).sort()}
...
function allOptionRows(){return DATA.unifiedRows.concat(DATA.operationRows||[])}
