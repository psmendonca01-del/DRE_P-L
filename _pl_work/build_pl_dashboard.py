...
def load_budget_rows():
    ...
    for excel_row, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        rows.append(item)
    return compact_finance_rows(rows)
...
function renderBudgetKpis(){const periods=currentPeriods(),actual=sumRows(scopedFinance(periods)),budget=sumRows(scopedBudget(periods));const variance=(key)=>(actual[key]||0)-(budget[key]||0),pct=(key)=>variationPct(actual[key]||0,budget[key]||0);const cards=[
['Receita Líquida Real.',fmtMoney(actual.netRevenue),'Realizado','var(--blue)',actual.netRevenue],
['Receita Líquida Budget',fmtMoney(budget.netRevenue),'Budget Média 4M','var(--green)',budget.netRevenue],
...
function renderBudgetByClient(){const groups=new Map();for(const r of filteredBudgetRows(currentPeriods())){if(!groups.has(r.client))groups.set(r.client,[]);groups.get(r.client).push(r)}const actualGroups=new Map();for(const r of filteredDetailRows(currentPeriods())){if(!actualGroups.has(r.client))actualGroups.set(r.client,[]);actualGroups.get(r.client).push(r)}const names=[...new Set([...groups.keys(),...actualGroups.keys()])].filter(Boolean);const data=names.map(name=>{const a=financeFromDetails(actualGroups.get(name)||[]),b=financeFromDetails(groups.get(name)||[]);return {name,a:a.netRevenue||0,b:b.netRevenue||0,d:(a.netRevenue||0)-(b.netRevenue||0)}}).filter(x=>Math.abs(x.a)>0||Math.abs(x.b)>0).sort((x,y)=>Math.abs(y.d)-Math.abs(x.d)).slice(0,12);$('budgetClientTable').innerHTML=`<thead><tr><th>Cliente</th><th>Realizado RL</th><th>Budget RL</th><th>Variação</th></tr></thead><tbody>${data.map(r=>`<tr><td>${displayLabel(r.name)}</td><td class="${cls(r.a)}">${fmtMoney(r.a)}</td><td class="${cls(r.b)}">${fmtMoney(r.b)}</td><td class="${cls(r.d)}">${fmtMoney(r.d)}</td></tr>`).join('')}</tbody>`}
